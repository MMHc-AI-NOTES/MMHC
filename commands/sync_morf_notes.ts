...
function getSortableTime(data: any, morfCreatedAt: DateTime): number {
  const dateStr =
    data?.Date ?? data?.date ?? data?.LastModified ?? data?.last_modified;
...
function buildSessionStringFromQuestions(
  data: any,
  sessionType?: number,
): string {
...
function resolveNoteTypeFromMorfData(
  data: any,
  noteId: string,
): ResolvedSessionType {
  // NoteName is where the real type label lives; Type/type are fallbacks.
  const typeValue =
    data?.NoteName ?? data?.noteName ?? data?.Type ?? data?.type;
...
export default class SyncMorfNotes extends BaseCommand {
  ...
  async run() {
    ...
    for (const row of rows) {
      ...
      const patientIdRaw =
        data.PatientId ?? data.patient_id ?? data.patientId ?? null;
      const clientId =
        data.ClientId ??
        data.client_id ??
        data.clientId ??
      ...
      if (
        ...
        const p = await Patient.query()
          .where("id", Number(patientIdRaw))
          .first();
        if (p) patientId = p.id;
      }
      if (
        ...
        const p = await Patient.query()
          .where("client_id", String(clientId).trim())
          .first();
        if (p) patientId = p.id;
      }
      if (patientId === null) {
        this.logger.warning(
          `Row id=${row.id}: no PatientId or ClientId, skipping`,
        );
        continue;
      }
    ...
    const sortedPatients = patientOrder.map(
      (key) => [key, byPatient.get(key)!] as const,
    );
    let created = 0;
    let updated = 0;
    let skipped = 0;
    ...
    for (const [pidStr, notes] of sortedPatients) {
      ...
      if (!patient) {
        this.logger.warning(
          `Patient id=${patientId}: not found, skipping ${notes.length} notes`,
        );
        ...
        try {
          ...
          if (
            ...
            const u = await User.query()
              .where("pq_id", String(practitionerIdRaw).trim())
              .first();
            if (u) practitionerId = u.id;
          }
          if (practitionerId === null) {
            const byEmail =
              data.PractitionerEmail ?? data.practitioner_email ?? data.Email;
            if (byEmail) {
              const u = await User.query()
                .where("email", String(byEmail).trim())
                .first();
              if (u) practitionerId = u.id;
            }
          }
          ...
          if (existing) {
            ...
            existing.sessionTime = sessionTime.isValid
              ? sessionTime
              : morf.createdAt;
          ...
          errors++;
          this.logger.error(
            `Row id=${morf.id} noteId=${noteId}: ${err.message}`,
          );
        }
      }
