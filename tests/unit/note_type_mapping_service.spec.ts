...
test.group("note_type_mapping_service | resolveSessionType", () => {
  test("resolves all four confirmed note-type labels", ({ assert }) => {
    ...
    assert.deepEqual(
      resolveSessionType("Initial Consultation: Intake/Assessment"),
      {
    ...
    assert.deepEqual(
      resolveSessionType("Initial Consultation: Assessment/Treatment Plan"),
      {
  ...
  test("is case-insensitive and trims whitespace", ({ assert }) => {
    assert.equal(
      resolveSessionType("  termination note  ").type,
      SessionTypeEnum.termination,
    );
    assert.equal(
      resolveSessionType("PROGRESS NOTE").type,
      SessionTypeEnum.progress_note,
    );
  });

  test("falls back via fuzzy substring match for label variants", ({
    assert,
  }) => {
    assert.equal(
      resolveSessionType("Progress Note - Adult").type,
      SessionTypeEnum.progress_note,
    );
    assert.equal(
      resolveSessionType(
        "Initial Consultation: Assessment/Treatment Plan (Revised)",
      ).type,
      SessionTypeEnum.treatment_plan,
    );
    assert.equal(
      resolveSessionType("Client Discharge Summary").type,
      SessionTypeEnum.termination,
    );
  });
...
test.group("note_type_mapping_service | buildSessionObject", () => {
  test("maps progress-note ids to their canonical field names", ({
    assert,
  }) => {
  ...
  test("maps intake-note ids to their canonical field names", ({ assert }) => {
    const result = buildSessionObject(
      [
        {
          id: "h08z-1",
          text: "Presenting Problem & Symptoms",
          answer: "Anxiety.",
        },
        {
          id: "kxgx-1",
          text: "Risk Assessment",
          answer: "No risk identified.",
        },
      ],
  ...
  test("the same question id resolves differently depending on note type (id collision)", ({
    ...
    const progress = buildSessionObject(
      [
        {
          id: "zad8-1",
          text: "Assessment & Therapeutic Intervention",
          answer: "A",
        },
      ],
      SessionTypeEnum.progress_note,
    ...
    assert.deepEqual(progress, {
      "Assessment & Therapeutic Intervention": "A",
    });
    assert.deepEqual(intake, { "Tenative Goals and Plans": "B" });
  });

  test("termination-note ids map to their canonical field names", ({
    assert,
  }) => {
    const result = buildSessionObject(
      [
  ...
  test("treatment-plan (no id map) falls back to question text", ({
    assert,
  }) => {
    const result = buildSessionObject(
  ...
  test("repeated field labels across goal blocks are preserved, not overwritten", ({
    assert,
  }) => {
  ...
  test("blank question text falls back to the raw id, not an empty key", ({
    assert,
  }) => {
    const result = buildSessionObject(
  ...
  test("an unmatched/unknown type does not apply the progress-note id map", ({
    assert,
  }) => {
    ...
    const result = buildSessionObject(
      [
        {
          id: "zad8-1",
          text: "Some Unrelated Field On An Unknown Note Type",
          answer: "X",
        },
      ],
      undefined,
    ...
    assert.deepEqual(result, {
      "Some Unrelated Field On An Unknown Note Type": "X",
    });
  });

  test("non-string answers (boolean, number, array) are formatted to strings", ({
    assert,
  }) => {
    const result = buildSessionObject([
      ...
      { id: "b", text: "Count Field", answer: 0 as any },
      {
        id: "c",
        text: "Multi Select",
        answer: ["Option A", "Option B"] as any,
      },
    ]);
    assert.deepEqual(result, {
