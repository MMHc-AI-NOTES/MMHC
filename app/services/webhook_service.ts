...
export const updateSessionCptCodeBySessionId = async (sessionId: number) => {
  ...
  const appointmentTypeId = await fetchAppointmentTypeIdFromBigQuery(
    session.noteId,
  );
...
function getSessionTimeFromPayload(
 ...
 */
export const createSessionFromWebhook = async (
  payload: webhookSessionValidatorInterface,
) => {
  ...
  try {
    ...
    if (!cptCode) {
      throw new Error(
        "CPT code 90791 not found. Please ensure CPT code seeder has been run.",
      );
    }
    ...
    if (payload.ClientId) {
      ...
      const patient = await Patient.query()
        .where("client_id", clientIdString)
        .first();
    ...
    if (existingSession) {
      ...
      existingSession.sessionTime = sessionTime.isValid
        ? sessionTime
        : DateTime.now();
...
const validatePayloadQuestions = (
  ...
  if (
    ...
    sessionType !== SessionTypeEnum.progress_note
  ) {
...
export const processWebhookJob = async (jobData: WebhookJobData) => {
  ...
  if (webhookValidation.isValid && noteId) {
    questionsValidation = validatePayloadQuestions(
      payload.Questions ?? [],
      sessionType,
    );
