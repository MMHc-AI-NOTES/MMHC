...
- **Node.js** — find the directory holding the customer's own source (commonly `src/`, or `main`/`exports` in `package.json`) and build a path glob `**/<app-dir>/src/**`. Remember it matches the absolute _runtime_ path, so anchor on a suffix that survives the build/deploy (the `**/` prefix), not the repo-relative path.
